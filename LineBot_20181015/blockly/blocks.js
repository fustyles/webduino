...
Blockly.Blocks['linebot_type'] = {
  init: function() {
    ...
    this.appendDummyInput()
        .appendField("Type")
        .appendField(new Blockly.FieldDropdown([["text (text)","text"], ["sticker (packageId, stickerId, [Notify]text)","sticker"], ["image (originalContentUrl, previewImageUrl, [Notify]text)","image"], ["video (originalContentUrl, previewImageUrl)","video"], ["audio (originalContentUrl, duration)","audio"], ["location (title, address, latitude, longitude)","location"]]), "value_type");
