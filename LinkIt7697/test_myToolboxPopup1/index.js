 ...
 * @Update 7/4/2023 21:30 (Taiwan Standard Time)
 */
function init() {
    ...
    function getMousePosition(e) {
        ...
        if (newBlock) {
            try {
                ...
                newBlock.moveBy(blockToMouseXY.x - 30 / (secondaryWorkspace.scale>=1?secondaryWorkspace.scale:10000000), blockToMouseXY.y - 8 / (secondaryWorkspace.scale>=1?secondaryWorkspace.scale:10000000));
            }
    ...
    function primaryWorkspaceListener(event) {
        if (event.type == "create") {
            ...
            if (block) {
                ...
                block = Blockly.utils.xml.textToDom('<xml xmlns="https://developers.google.com/blockly/xml">' + Blockly.Xml.domToText(block) + '</xml>');
    ...
    function getBlockToCenterXY(block) {
        ...
        var x = position.x;
        var y = position.y;
        var scrollX = secondaryWorkspace.scrollX / secondaryWorkspace.scale;
        var scrollY = secondaryWorkspace.scrollY / secondaryWorkspace.scale;
        var wsWidth = secondaryWorkspace.getParentSvg().width.baseVal.value / secondaryWorkspace.scale;
        var wsHeight = secondaryWorkspace.getParentSvg().height.baseVal.value / secondaryWorkspace.scale;
        var blockToCenterXY = {};
