    ...
    if (userMessage.toLowerCase()=="help") {
      ...
      if (userMessage.indexOf("image:")!=-1) {
        ...
        if (spreadsheet_ID&&sheet_Name_chatgpt)
          addDataToSpreadsheet(spreadsheet_ID, sheet_Name_chatgpt, replyToken, "user", userMessage);
        ...
        if (driveFolderName) {
          ...
          if (spreadsheet_ID&&sheet_Name_chatgpt)
            addDataToSpreadsheet(spreadsheet_ID, sheet_Name_chatgpt, replyToken, "assistant", imageUrl);
        } else {
          if (spreadsheet_ID&&sheet_Name_chatgpt)
            addDataToSpreadsheet(spreadsheet_ID, sheet_Name_chatgpt, replyToken, "assistant", openAI_response);
        }        

      } else {
        
        if (scriptProperties.getProperty(replyToken)==""||scriptProperties.getProperty(replyToken)==null) {
          openAI_historical_messages = [{"role": "system", "content": openAI_assistant_behavior}];

          if (spreadsheet_ID&&sheet_Name_chatgpt)
            addDataToSpreadsheet(spreadsheet_ID, sheet_Name_chatgpt, replyToken, "system", openAI_assistant_behavior);
        }
        else
          openAI_historical_messages = JSON.parse(scriptProperties.getProperty(replyToken)); 
        ...
        scriptProperties.setProperty(replyToken, JSON.stringify(openAI_historical_messages));
      ...
      scriptProperties.setProperty(replyToken, '');
      openAI_response = reset_response;

      if (spreadsheet_ID&&sheet_Name_chatgpt)
        addDataToSpreadsheet(spreadsheet_ID, sheet_Name_chatgpt, replyToken, "user", reset_command);      
    }
