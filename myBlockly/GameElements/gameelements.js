// Author: Chung-Yi Fu (Kaohsiung, Taiwan)   https://www.facebook.com/francefu
  ...
  function image_set(input_id,input_property,input_value) {
    if (document.getElementById("gameimg_"+input_id))
    {
	  var obj = document.getElementById("gameimg_"+input_id);
      if (input_property=='url')
        obj.src = input_value;
      else if (input_property=='width')
        obj.style.width = input_value + 'px';
  ...
  function image_collision(input_id1,input_id2,input_position) {
    if ((document.getElementById("gameimg_"+input_id1))&&(document.getElementById("gameimg_"+input_id2)))
    {
	  ...
	  if ((y1_h<=(y2+y2_h)/2)||(y1_h-y2>(y2_h-y2)/2))
            return 1;
	  else
	    return 0;
        }      
        else if (input_position=="down") {
	  if ((y1>(y2+y2_h)/2)||(y2_h-y1>(y2_h-y2)/2))
            return 1;
	  else
	    return 0;      
        }
        else if (input_position=="left") {
	  if ((x1_w<=(x2+x2_w)/2)||(x1_w-x2>(x2_w-x2)/2))
            return 1;
	  else
	    return 0;    
        } 
        else if (input_position=="right") {
	  if ((x1>(x2+x2_w)/2)||(x2_w-x1>(x2_w-x2)/2))
            return 1;
	  else
	    return 0;     
        }
