    ...
    if ((input_trcount>=1)&&(input_tdcount>=1)){
      ...
      for (var i=0;i<input_trcount;i++){
        ...
        for (var j=0;j<input_tdcount;j++){
          ...
          td.setAttribute("ondrop","javascript:var obj=document.getElementById(event.dataTransfer.getData('text/plain'));obj.style.position='static';obj.style.left=null;obj.style.top=null;event.preventDefault();alert(event.target.tagName);if(event.target.tagName!='td') {return false;} else {event.target.appendChild(obj);}");
