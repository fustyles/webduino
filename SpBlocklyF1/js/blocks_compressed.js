...
Blockly.Blocks.initializes_loop={
	...
	,onchange: function(event) {
		...
		if (event.type=="move") {
			var blocks = this.workspace.getBlocksByType("initializes_setup");
			if (blocks.length>1) {
				for (var i=1;i<blocks.length;i++) {
					blocks[i].dispose();
				}
			}
			var blocks = this.workspace.getBlocksByType("initializes_loop");
			if (blocks.length>1) {
				for (var i=1;i<blocks.length;i++) {
					blocks[i].dispose();
				}
			}
			var blocks = this.workspace.getBlocksByType("board_initializes_setup");
			if (blocks.length>1) {
				for (var i=1;i<blocks.length;i++) {
					blocks[i].dispose();
				}
