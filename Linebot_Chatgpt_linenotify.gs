...
let openAI_search_columnName = "「編號」、「姓名」、「處室」、「職稱」"; // 關鍵字查詢欄位清單
let openAI_assistant_behavior = `
請回覆陣列格式資料符合以下規範：
(1)請分析使用者對話內容，區分要傳送的訊息內容與傳送對象(陣列格式資料中與欄位${openAI_search_columnName}任一相同或相關聯，或所有對象皆傳送)
(2)對話內容中提及對象與欄位${openAI_search_columnName}任一相同或相關連，視為同一對象。
