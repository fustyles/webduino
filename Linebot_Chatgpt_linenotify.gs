...
let openAI_assistant_behavior = "請分析使用者訊息，判別要傳送的Line訊息與傳送的對象(編號、姓名、暱稱任一相同或所有人)，依照提供的資料格式字串將要傳送對象的訊息填入欄位[訊息]的值，若非傳送對象不列出，若無任何對象或訊息請回傳'請輸入要傳送的訊息與對象不須參照資料格式。只回覆資料格式字串不要多作解釋。資料格式字串如下：";
