...
const error_message = "請傳送文字訊息包含行事曆所需資料：日期、時間、持續時間(可無)、事項描述並聲明要新增行事曆，或者提供的openAI Key無法使用！";
...
function doPost(e) {
  if (e.postData) {
    ...
    if (userType=="text") {
      ...
      let jsonData = sendMessageToOpenaiChat(groq_api_KEY, openAI_messages).replace(/```json|```/g, "").trim();     
      if (jsonData!="error") {             
        try {
